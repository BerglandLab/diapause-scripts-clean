...
bamDir=$topDirectory/output/bams$reconstructionGroup

# Chromosomes to reconstruct
...
RABBITpackageLocation="${topDirectory}/etc/RABBIT_Packages/"
topNfounders=14     #topNfounders indicates that X most likely founders are chosen for performing higher-resolution imputation
...
if [[ ! -f "$ind_id".bam.stats ]]; then
    ...
    ${topDirectory}/etc/samtools idxstats $bamDir/${ind_id}.bam > ${ind_id}.bam.stats
fi
...
if [[ ! -f "$workDir/$ind_id.14.mlp" ]]; then

Rscript - <<EOF > $ind_id.14.mlp
#!/usr/bin/env Rscript
...
harp.freqs.long[, q99 := quantile(freq, 0.95, na.rm=TRUE), by=chromosome]

mlp <- harp.freqs.long[, .N, by=list(chromosome, lineID, freq>=q99)][freq==TRUE][order(chromosome,-N)][,c("chromosome","lineID","N")][N>1]
...
if [[ ! -f "$workDir/$ind_id.estimate.14.haps" ]]; then
...
mlp <- fread(paste(ind_id, ".14.mlp", sep=""), header=TRUE, showProgress=FALSE)
...
chromosome=$(echo $filename | cut -d "." -f 5)
...
echo -e "chromosome\tstart\tstop\tpar1\tpar2" > "${ind_id}.estimate.14.haps"
...
if [[ ${nLines} -eq 1 ]]; then
    ...
    chromosome=$(echo $filename | cut -d "." -f 5)
    echo -e "${chromosome}\t1\t25000000\t${par}\t${par}" >> "${ind_id}.estimate.14.haps"
else
python - ${filename} ${ind_id} <<EOF >> "${ind_id}.estimate.14.haps"
#!/usr/bin/env python
