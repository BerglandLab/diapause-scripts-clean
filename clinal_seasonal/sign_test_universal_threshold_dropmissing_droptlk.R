...
y<-foreach(pop=files$V1[1:3000], phenotype=files$V2[1:3000],draw=files$V3[1:3000], perm=files$V4[1:3000], model=files$V6[1:3000])%dopar%{
    ...
    gwas[,drop:=F]
    gwas[chr=="X"&pos>3600000&pos<3700000, drop:=T]
    gwas<-gwas[drop==F]
...
save(y, file="/scratch/pae3g/revisions/evolution/bergland2014_sign_universal_threshold_dropmissing_droptlk.Rdata")
...
pdf("/scratch/pae3g/revisions/figures/bergland2014_main_dropmissing_droptlk.pdf", height=8, width=5)
a.plot
dev.off()
