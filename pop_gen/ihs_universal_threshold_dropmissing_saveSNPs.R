...
files<-files[V4==0&V6=="nonloco"&V2=="diapause.bin"]


    
    # j<-foreach(pop=files$V1, phenotype=files$V2,draw=files$V3, perm=files$V4, model=files$V6, .errorhandling="remove")%dopar%{
    ...
    #   cutoff=-2
    #   e<-d[q<=-2]
    #   save(e, file=paste0("/scratch/pae3g/revisions/evolution/ihs_dgrp_perm", perm, "_draw", draw, "_pop", pop, "_st8_top1percent.Rdat"))
...
k<-foreach(pop=files$V1, phenotype=files$V2,draw=files$V3, perm=files$V4, model=files$V6, .errorhandling="remove")%dopar%{
  print(paste(draw, perm, sep=","))
  #read gwas adn do some fixes
  load(paste("/scratch/pae3g/revisions/genesis_", phenotype, "_draw", draw, "_perm", perm, "_pop", pop, "_" , model, "_allsnpgrm_wolbachia_dropmissing.Rdat", sep=""))
  gwas<-assoc.results   
  ...
  setnames(d, c("CHR","POSITION", "LOGPVALUE"), c("chr", "pos",'ihs.p'))
  e<-d[q<=(-2)]
  save(e, file=paste0("/scratch/pae3g/revisions/evolution/ihs_north_perm", perm, "_draw", draw, "_pop", pop, "_st8_top1percent.Rdat"))
...
#save output   
#output=paste0("/scratch/pae3g/revisions/evolution/ihs_north_universal_threshold_dropmissing.Rdat")
#print(output)
#save(k, file=output)
