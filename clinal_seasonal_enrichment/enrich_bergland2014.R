...
#function to do all enrichment calculations
enrich<-function(gwas, snpset, cline, seas, af, clinal.th.list, seas.th.list, seas.fixed.th, gwas.th.list, cline_polar, seas_polar, min.maf=0, max.maf=.5, output){
    ...
    if(!is.null(clinal.th.list)){
        ...
        clinal<-foreach(clinal.q.thresh = clinal.th.list, .errorhandling="remove")%dopar%{
            ...
            cline.enrich<-data.table(TT=sum(b.g$q.c<=clinal.q.thresh, na.rm=T), #true number of overlaps
                                     TF=sum(b.g$q.c>clinal.q.thresh, na.rm=T), #overlaps sthat are not signficant at threshold
                                     TT.conc=sum(b.g$q.c<=clinal.q.thresh&sign(b.g$gwas.Score)==sign(b.g$clinal.beta), na.rm=T), #number of overlaps that are also concordant
                                     TF.conc=sum(b.g$q.c>clinal.q.thresh&sign(b.g$gwas.Score)==sign(b.g$clinal.beta), na.rm=T),
                                     clinal.th=clinal.q.thresh, #number of overlaps that are concordant but not signfiicant
                                     gwas.th=top, #threhold
                                     seas.th=1, #threshold
                                     test="cline")
...
clump.enrich<-foreach(perm =c(0,101:200)) %do%{ #cycle through original data and 100 permutations
    ...
    a<-foreach (top = c(101, 490, 5316, 25250, 67914)) %do% { #these numbers correspond to the top # of snps in our different FDR cutoffs
        ...
        cline<-"/scratch/pae3g/evolution/bergland_2014_cline_clean.txt"
        seas<-"/scratch/pae3g/evolution/bergland_2014_seasonal_clean.txt"        
        freqs<-"/scratch/pae3g/evolution/bergland_2014_freqs_clean.txt"
        ...
        enrich(gwas=gwas, #gwas output file
               snps=snps, #unlinked index snps to test
               cline=cline, #clinal p-values
               seas=seas, #seasonal p-values
               af=freqs, #allele frequency data for filtering
               clinal.th.list=seq(from=-5, to=-.8, by=0.1), #list of log-scaled quantile thresholds to test
               seas.th.list=seq(from=-5, to=-.8, by=0.1), #list of log-scaled quantile thresholds to test
               seas.fixed.th=log10(.05), #fixed seasonal threhold to be used in clinal + seasonal test
               gwas.th.list=seq(from=-5, to=-.8, by=0.1), #not used in this iteration, using top snps insteas
               cline_polar=-1, #polarize clinal allele freqs if needed
               seas_polar=-1, #polarize seasona
               min.maf=0, #minimum allele frequency to test
               max.maf=0.5, #maximum allele frequency to test
...
#sae for downstream analysis
write.table(clump.enrich, "/scratch/pae3g/evolution/bergland2014_enrich_intersectfirst_clump.txt", quote=F, row.names=F, sep="\t")
...
a<-ggplot(perm.sum[test=="cline"&!is.na(prop.enrich)], aes(x=as.factor(gwas.th), y=-1*clinal.th, fill=prop.enrich))+
    geom_tile()+
    scale_fill_viridis(option="viridis", limits=c(-1,3.75))+
...
pdf("/scratch/pae3g/clinal_seasonal_2014_clump_enrich.pdf", height=4, width=8)
