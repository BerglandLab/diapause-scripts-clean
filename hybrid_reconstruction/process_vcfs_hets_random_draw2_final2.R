...
print(seed)
#set seed and use dorng to ensure reproducibility
set.seed(seed)

#read in a file with "bad paths"
...
import_data_mask<-function(fn){
  ...
  if (!file.size(fn) == 0) {
    ...
    #randomly choose a parental genotype when parent is het
...
foreach(swarm=c("A", "B"))%do%{
  ...
  #split files into groups to prevent memory issues
  groups <- split(file_list, ceiling(seq_along(file_list)/ceiling(length(file_list)/16)))
  #read in the original vcf with all sites
