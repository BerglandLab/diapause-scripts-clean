...
if(!file.exists(paste0(outdir, "lasso_", phenotype, "_draw",draw, "_perm", perm, "_", model, "_pop", pop, ".random.lassoSites.Rdata"))){
  print("running lasso")
  maf.threshold=0.05
  ...
  #rank GWAS RANDOMLY
  gwas[,r:=sample(nrow(gwas),nrow(gwas),replace=F)]
  ...
  print("saving output")
  pred.out.fn <- paste0(outdir, "lasso_", phenotype, "_draw",draw, "_perm", perm, "_", model, "_pop", pop, ".random.lassoPred.Rdata")
  
  sites.out.fn <-   paste0(outdir, "lasso_", phenotype, "_draw",draw, "_perm", perm, "_", model, "_pop", pop, ".random.lassoSites.Rdata")
  
  fits.out.fn <-  paste0(outdir, "lasso_", phenotype, "_draw",draw, "_perm", perm, "_", model, "_pop", pop, ".random.lassoData.Rdata")
