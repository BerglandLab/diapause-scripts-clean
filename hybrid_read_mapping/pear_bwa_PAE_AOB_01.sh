...
#define a function that will run reads through pear and bwa
	mapReads () {
		...
		## define some parameters
			...
			inputDir=${2}
			threads=10
			outputDir="/mnt/pricey_2/priscilla/hybrid/mapped/PAE_AOB_01"
			run="PAE_AOB_01"
			lane="PAE_AOB_01"
			strain=$(sed -n ''${sample}''p ${inputDir}/raw_data/PAE_AOB_01/PAE_AOB_01_filenames.txt)
			echo "strain=" $strain

			echo 'starting PEAR'
			pear \
					-f ${inputDir}/raw_data/PAE_AOB_01/${sample}_R1_001.fastq.gz \
					-r ${inputDir}/raw_data/PAE_AOB_01/${sample}_R2_001.fastq.gz \
					-o ${inputDir}/raw_data/PAE_AOB_01/${strain} \
					-j ${threads}
			...
			bwa mem -t ${threads} \
					...
					${inputDir}/raw_data/PAE_AOB_01/${strain}.assembled.fastq | \
					samtools view -Suh - | \
			...
			echo 'mapping unassembled reads with bwa'			
					## unassembled reads
						bwa mem -t ${threads} \
							...
							/mnt/internal_2/priscilla/ref_files/all_dmel.fasta \
							 ${inputDir}/raw_data/PAE_AOB_01/${strain}.unassembled.forward.fastq \
							 ${inputDir}/raw_data/PAE_AOB_01/${strain}.unassembled.reverse.fastq | \
						samtools view -Suh - | \
