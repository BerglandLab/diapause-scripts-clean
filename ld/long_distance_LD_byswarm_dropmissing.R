...
geno <- snpgdsOpen("/scratch/pae3g/genome-reconstruction/final2_draw1_replaced.vcf.gds", allow.fork=T)
    ...
    print("hybrid swarm")
    foreach(swarm=c("A", "B", "both"))%do%{
        phenos <- fread("/scratch/pae3g/revisions/phenos_wolbachia_missinggeno.txt")
        
        phenos <- phenos[n.chr.imp==5&prop.unknown<=0.05]
        
        if(swarm=="both"){
            samps.to.use=phenos$sample.id
        }
        else{
            samps.to.use=phenos[swarm==swarm, sample.id]
        }        
    a<-snpgdsSNPList(geno, sample.id=samps.to.use)
    ...
    m<-foreach(draw=c(1:10000), .errorhandling="remove")%dopar%{
        ...
        y<-foreach(chrom=c(2,3))%do%{
            ...
            ld<-snpgdsLDMat(geno, slide=-1,verbose=F,snp.id = c(focal.snp, match.snp), method='composite', sample.id=samps.to.use)$LD[2,1]
    ...
    write.table(m, paste0("/scratch/pae3g/revisions/chromsome_arm_ld_", swarm, ".txt"), quote=F, sep="\t", row.names=F)
    ...
    write.table(m, paste0("/scratch/pae3g/revisions/interchromosomal_ld_", swarm, ".txt"), quote=F, sep="\t", row.names=F)
}
