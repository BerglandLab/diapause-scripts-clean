...
y<-foreach(dist=c(50,100,500,1000,5000,10000,50000,100000,500000,1000000,5000000,10000000), .errorhandling = "remove")%dopar%{   
    ...
    while (nsampled<=10000){
        ...
        ld<-snpgdsLDMat(geno, slide=-1,verbose=F,snp.id = c(focal.snp, focal.snp2), sample.id=sample(samps, 34), method='composite')$LD[2,1]
...
write.table(y, "/scratch/pae3g/evolution/ld_decay_dgrp_mafover0.05_downsampled_revised34.txt", quote=F, sep="\t", row.names = F)
...
# y<-rbindlist(y)
# 
# write.table(y, "/scratch/pae3g/evolution/ld_decay_dgrp_mafless0.05_downsampled_revised34.txt", quote=F, sep="\t", row.names = F)